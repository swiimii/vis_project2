  ...
  constructor(_config, _data, _data_selection, max_bars=12, _legend) {
    ...
    initVis() {
      ...
      (Array.from(vis.data_map.keys()).sort(d3.ascending)).forEach((key) => {
        vis.data_selections.set(key, vis.data_map.get(key).length);
      });
    ...
    renderVis() {
      ...
      vis.chart.selectAll('category').data(vis.data_selections.keys()).enter()
        .append('rect')
          ...
          .attr('y', d => vis.yScale(vis.data_selections.get(d).length))
          .attr('height', d => vis.height - vis.yScale(vis.data_selections.get(d).length))
